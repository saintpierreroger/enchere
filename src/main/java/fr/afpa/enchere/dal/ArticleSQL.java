...
public class ArticleSQL {
    ...
    private static final String SELECT_ALL1 = "SELECT nom_article, prix_vente, date_fin_encheres, pseudo, utilisateurs.no_utilisateur" +
            " FROM articles_vendus " +
            " INNER JOIN utilisateurs " +
            "ON articles_vendus.no_utilisateur = utilisateurs.no_utilisateur";

    private static final String SELECT_BY_NOM_ARTICLE_ET_NO_CATEGORIE = "SELECT nom_article, prix_vente, date_fin_encheres, " +
    ...
    public List<UtilisateursArticles_Vendus> selectAll1() {
        ...
        try (Connection connection = ConnectionProvider.getConnection()) {
            ...
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                listeArticles.add(new UtilisateursArticles_Vendus(rs.getString("nom_article"), rs.getInt("prix_vente"), rs.getDate("date_fin_encheres"), rs.getString("pseudo"), rs.getString("no_utilisateur")));
            }
            connection.close();
        } catch (Exception e) {
            e.printStackTrace();
